...
const ACCESS_PASSWORD =
  process.env.ACCESS_PASSWORD || "bitte-aendern";

const DAILY_LIMIT = Math.max(
  1,
  Number(process.env.DAILY_LIMIT || 40)
);
...
/* LOGIN
   SameSite=None + Secure ist notwendig,
   damit die Sitzung auch im Wix-iframe funktioniert.
*/
app.post("/api/login", (req, res) => {
  if (
    String(req.body?.password || "") !== ACCESS_PASSWORD
  ) {
  ...
  res.setHeader(
    "Set-Cookie",
    `phil_session=${encodeURIComponent(
      token
    )}; HttpOnly; SameSite=None; Secure; Path=/; Max-Age=43200`
  );
  ...
  properties: {
    ...
    synthese: {
      ...
      properties: {
        problem: { type: "string" },
        antwort: { type: "string" },
        argumentationsweg: { type: "string" },
        philosophisch: { type: "string" },
        einfach: { type: "string" },
...
app.post(
  ...
  async (req, res) => {
    ...
    console.log(
      `[Analyse V3] gestartet (${String(
        req.body?.text || ""
      ).length} Zeichen)`
    );

    try {
      const { key, n } = bucket(req);

      if (n >= DAILY_LIMIT) {
        return res.status(429).json({
          error:
            "Das Tageslimit für diesen Zugang ist erreicht."
        });
      }
      ...
      if (text.length > 20000) {
        return res.status(400).json({
          error:
            "Bitte einen kürzeren Textabschnitt verwenden (max. 20.000 Zeichen)."
        });
      }
      ...
      console.log(
        `[Analyse V3] erfolgreich nach ${Math.round(
...
app.listen(
  ...
  () =>
    console.log(
      `Textmikroskop V3 läuft auf Port ${PORT}`
    )
);
